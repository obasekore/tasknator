...
$Page->showMessage();
?>
<form name="fuseradd" id="fuseradd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_user_role_id">
    <select
        ...
        data-select2-id="fuseradd_x_role_id"
        <?php } ?>
        data-table="user"
...
loadjs.ready("fuseradd", function() {
    var options = { name: "x_role_id", selectId: "fuseradd_x_role_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fuseradd.lists.role_id?.lookupOptions.length) {
        options.data = { id: "x_role_id", form: "fuseradd" };
    } else {
        options.ajax = { id: "x_role_id", form: "fuseradd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fuseradd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fuseradd", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fuseradd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fuseradd", "x_updated_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fuseradd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
