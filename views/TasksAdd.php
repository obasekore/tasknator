...
$Page->showMessage();
?>
<form name="fTasksadd" id="fTasksadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_Tasks_TaskerID">
    <select
        ...
        data-select2-id="fTasksadd_x_TaskerID"
        <?php } ?>
        data-table="Tasks"
...
<?php if (!$Page->TaskerID->IsNativeSelect) { ?>
<script>
loadjs.ready("fTasksadd", function() {
    var options = { name: "x_TaskerID", selectId: "fTasksadd_x_TaskerID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fTasksadd.lists.TaskerID?.lookupOptions.length) {
        options.data = { id: "x_TaskerID", form: "fTasksadd" };
    } else {
        options.ajax = { id: "x_TaskerID", form: "fTasksadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fTasksadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fTasksadd", "x_StartTime", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fTasksadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fTasksadd", "x_CreatedAt", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fTasksadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fTasksadd", "x_UpdatedAt", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fTasksadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fTasksadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
