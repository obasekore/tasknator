...
class Task extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // tasker_id
        $this->tasker_id = new DbField(
            $this, // Table
            'x_tasker_id', // Variable name
            'tasker_id', // Name
            '`tasker_id`', // Expression
            '`tasker_id`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`tasker_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->tasker_id->InputTextType = "text";
        $this->tasker_id->Raw = true;
        $this->tasker_id->Nullable = false; // NOT NULL field
        $this->tasker_id->Required = true; // Required field
        $this->tasker_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->tasker_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['tasker_id'] = &$this->tasker_id;
        ...
        $this->start_time = new DbField(
            ...
            CastDateFieldForLike("`start_time`", 0, "DB"), // Basic search expression
            135, // Type
            19, // Size
            0, // Date/Time format
            false, // Is upload field
        ...
        $this->start_time->Nullable = false; // NOT NULL field
        $this->start_time->Required = true; // Required field
        $this->start_time->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
        $this->start_time->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "task";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->tasker_id->DbValue = $row['tasker_id'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TaskList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "TaskView" => $Language->phrase("View"),
            "TaskEdit" => $Language->phrase("Edit"),
            "TaskAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TaskView",
            Config("API_ADD_ACTION") => "TaskAdd",
            Config("API_EDIT_ACTION") => "TaskEdit",
            Config("API_DELETE_ACTION") => "TaskDelete",
            Config("API_LIST_ACTION") => "TaskList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TaskView", $parm);
        } else {
            $url = $this->keyUrl("TaskView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TaskList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TaskList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TaskDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        // tasker_id
        $this->tasker_id->setupEditAttributes();
        $this->tasker_id->EditValue = $this->tasker_id->CurrentValue;
        $this->tasker_id->PlaceHolder = RemoveHtml($this->tasker_id->caption());
        if (strval($this->tasker_id->EditValue) != "" && is_numeric($this->tasker_id->EditValue)) {
            $this->tasker_id->EditValue = FormatNumber($this->tasker_id->EditValue, null);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->tasker_id);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->tasker_id);
