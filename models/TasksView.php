...
class TasksView extends Tasks
{
    ...
    // CSS class/style
    public $CurrentPageName = "TasksView";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("TaskID") ?? Route("TaskID")) !== null) {
            ...
            $this->RecKey["TaskID"] = $this->TaskID->QueryStringValue;
        } elseif (Post("TaskID") !== null) {
            $this->TaskID->setFormValue(Post("TaskID"));
            $this->RecKey["TaskID"] = $this->TaskID->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->TaskID->setQueryStringValue($keyValue);
            $this->RecKey["TaskID"] = $this->TaskID->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "TasksList"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "TasksList"; // No matching record, return to list
                    }
                break;
        }
        if ($returnUrl != "") {
