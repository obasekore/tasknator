...
class Services extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->ServiceID->IsAutoIncrement = true; // Autoincrement field
        $this->ServiceID->IsPrimaryKey = true; // Primary key field
        ...
        // ServiceName
        $this->ServiceName = new DbField(
            $this, // Table
            'x_ServiceName', // Variable name
            'ServiceName', // Name
            '`ServiceName`', // Expression
            '`ServiceName`', // Basic search expression
            200, // Type
            200, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`ServiceName`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->ServiceName->InputTextType = "text";
        $this->ServiceName->Nullable = false; // NOT NULL field
        $this->ServiceName->Required = true; // Required field
        $this->ServiceName->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['ServiceName'] = &$this->ServiceName;

        // ParentService
        $this->ParentService = new DbField(
            $this, // Table
            'x_ParentService', // Variable name
            'ParentService', // Name
            '`ParentService`', // Expression
            '`ParentService`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`ParentService`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->ParentService->addMethod("getDefault", fn() => 0);
        $this->ParentService->InputTextType = "text";
        $this->ParentService->Raw = true;
        $this->ParentService->Nullable = false; // NOT NULL field
        $this->ParentService->setSelectMultiple(false); // Select one
        $this->ParentService->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->ParentService->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->ParentService->Lookup = new Lookup($this->ParentService, 'Services', false, 'ServiceID', ["ServiceName","","",""], '', '', [], [], [], [], [], [], false, '', '', "`ServiceName`");
        $this->ParentService->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->ParentService->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['ParentService'] = &$this->ParentService;
        ...
        $this->Status->Lookup = new Lookup($this->Status, 'Services', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->ServiceID->setDbValue($conn->lastInsertId());
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['ServiceID']) && !EmptyValue($this->ServiceID->CurrentValue)) {
                $rs['ServiceID'] = $this->ServiceID->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('ServiceID', $rs)) {
                AddFilter($where, QuotedName('ServiceID', $this->Dbid) . '=' . QuotedValue($rs['ServiceID'], $this->ServiceID->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->ParentService->DbValue = $row['ParentService'];
    ...
    {
        $keys = [];
        $val = $current ? $this->ServiceID->CurrentValue : $this->ServiceID->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->ServiceID->CurrentValue = $keys[0];
            } else {
                $this->ServiceID->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('ServiceID', $row) ? $row['ServiceID'] : null;
        } else {
            $val = !EmptyValue($this->ServiceID->OldValue) && !$current ? $this->ServiceID->OldValue : $this->ServiceID->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@ServiceID@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ServicesList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ServicesView" => $Language->phrase("View"),
            "ServicesEdit" => $Language->phrase("Edit"),
            "ServicesAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ServicesView",
            Config("API_ADD_ACTION") => "ServicesAdd",
            Config("API_EDIT_ACTION") => "ServicesEdit",
            Config("API_DELETE_ACTION") => "ServicesDelete",
            Config("API_LIST_ACTION") => "ServicesList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ServicesView", $parm);
        } else {
            $url = $this->keyUrl("ServicesView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServicesEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServicesList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServicesAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServicesList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ServicesDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"ServiceID\":" . VarToJson($this->ServiceID->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->ServiceID->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->ServiceID->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("ServiceID") ?? Route("ServiceID")) !== null) {
                $arKeys[] = $keyValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->ServiceName);
                    $doc->exportCaption($this->ParentService);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->ServiceName);
                        $doc->exportField($this->ParentService);
