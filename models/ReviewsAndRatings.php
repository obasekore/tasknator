...
/**
 * Table class for ReviewsAndRatings
 */
class ReviewsAndRatings extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "ReviewsAndRatings";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // ReviewRatingID
        $this->ReviewRatingID = new DbField(
            $this, // Table
            'x_ReviewRatingID', // Variable name
            'ReviewRatingID', // Name
            '`ReviewRatingID`', // Expression
            '`ReviewRatingID`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`ReviewRatingID`', // Virtual expression
            false, // Is virtual
        ...
        $this->ReviewRatingID->InputTextType = "text";
        $this->ReviewRatingID->Raw = true;
        $this->ReviewRatingID->IsAutoIncrement = true; // Autoincrement field
        $this->ReviewRatingID->IsPrimaryKey = true; // Primary key field
        $this->ReviewRatingID->Nullable = false; // NOT NULL field
        $this->ReviewRatingID->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->ReviewRatingID->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['ReviewRatingID'] = &$this->ReviewRatingID;
        ...
        $this->Review->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->CreatedAt->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->UpdatedAt->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "ReviewsAndRatings";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->ReviewRatingID->setDbValue($conn->lastInsertId());
            $rs['ReviewRatingID'] = $this->ReviewRatingID->DbValue;
        }
        return $result;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['ReviewRatingID']) && !EmptyValue($this->ReviewRatingID->CurrentValue)) {
                $rs['ReviewRatingID'] = $this->ReviewRatingID->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('ReviewRatingID', $rs)) {
                AddFilter($where, QuotedName('ReviewRatingID', $this->Dbid) . '=' . QuotedValue($rs['ReviewRatingID'], $this->ReviewRatingID->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->ReviewRatingID->DbValue = $row['ReviewRatingID'];
    ...
    {
        return "`ReviewRatingID` = @ReviewRatingID@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->ReviewRatingID->CurrentValue : $this->ReviewRatingID->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->ReviewRatingID->CurrentValue = $keys[0];
            } else {
                $this->ReviewRatingID->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('ReviewRatingID', $row) ? $row['ReviewRatingID'] : null;
        } else {
            $val = !EmptyValue($this->ReviewRatingID->OldValue) && !$current ? $this->ReviewRatingID->OldValue : $this->ReviewRatingID->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@ReviewRatingID@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ReviewsAndRatingsList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ReviewsAndRatingsView" => $Language->phrase("View"),
            "ReviewsAndRatingsEdit" => $Language->phrase("Edit"),
            "ReviewsAndRatingsAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ReviewsAndRatingsView",
            Config("API_ADD_ACTION") => "ReviewsAndRatingsAdd",
            Config("API_EDIT_ACTION") => "ReviewsAndRatingsEdit",
            Config("API_DELETE_ACTION") => "ReviewsAndRatingsDelete",
            Config("API_LIST_ACTION") => "ReviewsAndRatingsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ReviewsAndRatingsView", $parm);
        } else {
            $url = $this->keyUrl("ReviewsAndRatingsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ReviewsAndRatingsAdd?" . $parm;
        } else {
            $url = "ReviewsAndRatingsAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ReviewsAndRatingsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ReviewsAndRatingsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ReviewsAndRatingsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ReviewsAndRatingsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ReviewsAndRatingsDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"ReviewRatingID\":" . VarToJson($this->ReviewRatingID->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->ReviewRatingID->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->ReviewRatingID->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("ReviewRatingID") ?? Route("ReviewRatingID")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->ReviewRatingID->CurrentValue = $key;
            } else {
                $this->ReviewRatingID->OldValue = $key;
            }
    ...
    {
        global $Response;
        $listPage = "ReviewsAndRatingsList";
    ...
    public function renderEditRow()
    {
        ...
        // ReviewRatingID
        $this->ReviewRatingID->setupEditAttributes();
        $this->ReviewRatingID->EditValue = $this->ReviewRatingID->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->ReviewRatingID);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->ReviewRatingID);
