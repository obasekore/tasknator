...
class Admin extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->role_id->Lookup = new Lookup($this->role_id, 'role', false, 'id', ["role_name","","",""], '', '', [], [], [], [], [], [], false, '', '', "`role_name`");
        ...
        $this->last_login = new DbField(
            ...
            CastDateFieldForLike("`last_login`", 0, "DB"), // Basic search expression
            135, // Type
            19, // Size
            0, // Date/Time format
            false, // Is upload field
        ...
        $this->last_login->Nullable = false; // NOT NULL field
        $this->last_login->Required = true; // Required field
        $this->last_login->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
        $this->last_login->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "AdminView" => $Language->phrase("View"),
            "AdminEdit" => $Language->phrase("Edit"),
            "AdminAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AdminView",
            Config("API_ADD_ACTION") => "AdminAdd",
            Config("API_EDIT_ACTION") => "AdminEdit",
            Config("API_DELETE_ACTION") => "AdminDelete",
            Config("API_LIST_ACTION") => "AdminList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AdminView", $parm);
        } else {
            $url = $this->keyUrl("AdminView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AdminEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AdminList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AdminList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("AdminDelete", $parm);
        }
    }
