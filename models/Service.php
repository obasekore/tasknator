...
/**
 * Table class for service
 */
class Service extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // parent
        $this->_parent = new DbField(
            $this, // Table
            'x__parent', // Variable name
            'parent', // Name
            '`parent`', // Expression
        ...
        $this->_parent->InputTextType = "text";
        $this->_parent->Raw = true;
        $this->_parent->Nullable = false; // NOT NULL field
        $this->_parent->Required = true; // Required field
        $this->_parent->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->_parent->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['parent'] = &$this->_parent;
        ...
        // options
        $this->options = new DbField(
            $this, // Table
            'x_options', // Variable name
            'options', // Name
            '`options`', // Expression
            '`options`', // Basic search expression
            201, // Type
            2147483647, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`options`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->options->InputTextType = "text";
        $this->options->Nullable = false; // NOT NULL field
        $this->options->Required = true; // Required field
        $this->options->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "service";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->_parent->DbValue = $row['parent'];
        $this->service_name->DbValue = $row['service_name'];
        $this->status->DbValue = $row['status'];
        $this->options->DbValue = $row['options'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ServiceList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ServiceView" => $Language->phrase("View"),
            "ServiceEdit" => $Language->phrase("Edit"),
            "ServiceAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ServiceView",
            Config("API_ADD_ACTION") => "ServiceAdd",
            Config("API_EDIT_ACTION") => "ServiceEdit",
            Config("API_DELETE_ACTION") => "ServiceDelete",
            Config("API_LIST_ACTION") => "ServiceList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ServiceView", $parm);
        } else {
            $url = $this->keyUrl("ServiceView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServiceEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServiceList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServiceAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ServiceList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ServiceDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->options->EditValue = $this->options->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->_parent);
                    $doc->exportCaption($this->service_name);
                    $doc->exportCaption($this->status);
                    $doc->exportCaption($this->options);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->_parent);
                        $doc->exportField($this->service_name);
                        $doc->exportField($this->status);
                        $doc->exportField($this->options);
