...
/**
 * Table class for TaskerService
 */
class TaskerService extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "TaskerService";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // TaskerServiceID
        $this->TaskerServiceID = new DbField(
            $this, // Table
            'x_TaskerServiceID', // Variable name
            'TaskerServiceID', // Name
            '`TaskerServiceID`', // Expression
            '`TaskerServiceID`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`TaskerServiceID`', // Virtual expression
            false, // Is virtual
        ...
        $this->TaskerServiceID->InputTextType = "text";
        $this->TaskerServiceID->Raw = true;
        $this->TaskerServiceID->IsAutoIncrement = true; // Autoincrement field
        $this->TaskerServiceID->IsPrimaryKey = true; // Primary key field
        $this->TaskerServiceID->Nullable = false; // NOT NULL field
        $this->TaskerServiceID->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->TaskerServiceID->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['TaskerServiceID'] = &$this->TaskerServiceID;
        ...
        $this->ServiceID->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['ServiceID'] = &$this->ServiceID;

        // AverageRating
        $this->AverageRating = new DbField(
            $this, // Table
            'x_AverageRating', // Variable name
            'AverageRating', // Name
            '`AverageRating`', // Expression
            '`AverageRating`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`AverageRating`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->AverageRating->addMethod("getDefault", fn() => 0);
        $this->AverageRating->InputTextType = "text";
        $this->AverageRating->Raw = true;
        $this->AverageRating->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->AverageRating->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['AverageRating'] = &$this->AverageRating;

        // ReviewCount
        $this->ReviewCount = new DbField(
            $this, // Table
            'x_ReviewCount', // Variable name
            'ReviewCount', // Name
            '`ReviewCount`', // Expression
            '`ReviewCount`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`ReviewCount`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->ReviewCount->addMethod("getDefault", fn() => 0);
        $this->ReviewCount->InputTextType = "text";
        $this->ReviewCount->Raw = true;
        $this->ReviewCount->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->ReviewCount->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        $this->Fields['ReviewCount'] = &$this->ReviewCount;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "TaskerService";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->TaskerServiceID->setDbValue($conn->lastInsertId());
            $rs['TaskerServiceID'] = $this->TaskerServiceID->DbValue;
        }
        return $result;
    }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['TaskerServiceID']) && !EmptyValue($this->TaskerServiceID->CurrentValue)) {
                $rs['TaskerServiceID'] = $this->TaskerServiceID->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('TaskerServiceID', $rs)) {
                AddFilter($where, QuotedName('TaskerServiceID', $this->Dbid) . '=' . QuotedValue($rs['TaskerServiceID'], $this->TaskerServiceID->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->TaskerServiceID->DbValue = $row['TaskerServiceID'];
        $this->_UserID->DbValue = $row['UserID'];
        $this->ServiceID->DbValue = $row['ServiceID'];
        $this->AverageRating->DbValue = $row['AverageRating'];
        $this->ReviewCount->DbValue = $row['ReviewCount'];
    ...
    {
        return "`TaskerServiceID` = @TaskerServiceID@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->TaskerServiceID->CurrentValue : $this->TaskerServiceID->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->TaskerServiceID->CurrentValue = $keys[0];
            } else {
                $this->TaskerServiceID->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('TaskerServiceID', $row) ? $row['TaskerServiceID'] : null;
        } else {
            $val = !EmptyValue($this->TaskerServiceID->OldValue) && !$current ? $this->TaskerServiceID->OldValue : $this->TaskerServiceID->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@TaskerServiceID@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TaskerServiceList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "TaskerServiceView" => $Language->phrase("View"),
            "TaskerServiceEdit" => $Language->phrase("Edit"),
            "TaskerServiceAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TaskerServiceView",
            Config("API_ADD_ACTION") => "TaskerServiceAdd",
            Config("API_EDIT_ACTION") => "TaskerServiceEdit",
            Config("API_DELETE_ACTION") => "TaskerServiceDelete",
            Config("API_LIST_ACTION") => "TaskerServiceList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TaskerServiceView", $parm);
        } else {
            $url = $this->keyUrl("TaskerServiceView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "TaskerServiceAdd?" . $parm;
        } else {
            $url = "TaskerServiceAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TaskerServiceEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TaskerServiceList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TaskerServiceAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TaskerServiceList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TaskerServiceDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"TaskerServiceID\":" . VarToJson($this->TaskerServiceID->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->TaskerServiceID->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->TaskerServiceID->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("TaskerServiceID") ?? Route("TaskerServiceID")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->TaskerServiceID->CurrentValue = $key;
            } else {
                $this->TaskerServiceID->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        // TaskerServiceID
        $this->TaskerServiceID->HrefValue = "";
        $this->TaskerServiceID->TooltipValue = "";
        ...
        $this->AverageRating->TooltipValue = "";

        // ReviewCount
        $this->ReviewCount->HrefValue = "";
        $this->ReviewCount->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // TaskerServiceID
        $this->TaskerServiceID->setupEditAttributes();
        $this->TaskerServiceID->EditValue = $this->TaskerServiceID->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->TaskerServiceID);
                    $doc->exportCaption($this->_UserID);
                    $doc->exportCaption($this->ServiceID);
                    $doc->exportCaption($this->AverageRating);
                    $doc->exportCaption($this->ReviewCount);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->TaskerServiceID);
                        $doc->exportField($this->_UserID);
                        $doc->exportField($this->ServiceID);
                        $doc->exportField($this->AverageRating);
                        $doc->exportField($this->ReviewCount);
