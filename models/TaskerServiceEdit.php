 ...
 * Page class
 */
class TaskerServiceEdit extends TaskerService
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TaskerServiceEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "TaskerServiceEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("TaskerServiceID") ?? Key(0) ?? Route(2)) !== null) {
                $this->TaskerServiceID->setQueryStringValue($keyValue);
                $this->TaskerServiceID->setOldValue($this->TaskerServiceID->QueryStringValue);
            } elseif (Post("TaskerServiceID") !== null) {
                $this->TaskerServiceID->setFormValue(Post("TaskerServiceID"));
                $this->TaskerServiceID->setOldValue($this->TaskerServiceID->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("TaskerServiceID") ?? Route("TaskerServiceID")) !== null) {
                    ...
                    $this->TaskerServiceID->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->TaskerServiceID->IsDetailKey) {
            $this->TaskerServiceID->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->TaskerServiceID->CurrentValue = $this->TaskerServiceID->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->TaskerServiceID->Visible && $this->TaskerServiceID->Required) {
                if (!$this->TaskerServiceID->IsDetailKey && EmptyValue($this->TaskerServiceID->FormValue)) {
                    $this->TaskerServiceID->addErrorMessage(str_replace("%s", $this->TaskerServiceID->caption(), $this->TaskerServiceID->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // AverageRating
        $this->AverageRating->setDbValueDef($rsnew, $this->AverageRating->CurrentValue, $this->AverageRating->ReadOnly);

        // ReviewCount
        $this->ReviewCount->setDbValueDef($rsnew, $this->ReviewCount->CurrentValue, $this->ReviewCount->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['AverageRating'])) { // AverageRating
            $this->AverageRating->CurrentValue = $row['AverageRating'];
        }
        if (isset($row['ReviewCount'])) { // ReviewCount
            $this->ReviewCount->CurrentValue = $row['ReviewCount'];
        }
