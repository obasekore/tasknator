 ...
 * Page class
 */
class ReviewsAndRatingsEdit extends ReviewsAndRatings
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ReviewsAndRatingsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "ReviewsAndRatingsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("ReviewRatingID") ?? Key(0) ?? Route(2)) !== null) {
                $this->ReviewRatingID->setQueryStringValue($keyValue);
                $this->ReviewRatingID->setOldValue($this->ReviewRatingID->QueryStringValue);
            } elseif (Post("ReviewRatingID") !== null) {
                $this->ReviewRatingID->setFormValue(Post("ReviewRatingID"));
                $this->ReviewRatingID->setOldValue($this->ReviewRatingID->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("ReviewRatingID") ?? Route("ReviewRatingID")) !== null) {
                    ...
                    $this->ReviewRatingID->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->ReviewRatingID->IsDetailKey) {
            $this->ReviewRatingID->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->ReviewRatingID->CurrentValue = $this->ReviewRatingID->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->ReviewRatingID->Visible && $this->ReviewRatingID->Required) {
                if (!$this->ReviewRatingID->IsDetailKey && EmptyValue($this->ReviewRatingID->FormValue)) {
                    $this->ReviewRatingID->addErrorMessage(str_replace("%s", $this->ReviewRatingID->caption(), $this->ReviewRatingID->RequiredErrorMessage));
                }
            }
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['TaskID'])) { // TaskID
            $this->TaskID->CurrentValue = $row['TaskID'];
        }
        if (isset($row['Review'])) { // Review
            $this->Review->CurrentValue = $row['Review'];
        }
